...
def parse_args() -> argparse.Namespace:
    ...
    '--include-mutations',
    nargs='*',
    default=A1_MUTATIONS,
    help='Mutations to include in the A1 comparison.',
)
    parser.add_argument(
        '--exp-ids',
        nargs='*',
...
def prepare_block_table(meta: pd.DataFrame, available_blocks: pd.DataFrame, args: argparse.Namespace) -> pd.DataFrame:
    ...
    if args.include_mutations:
        block_table = block_table.loc[block_table['Mutation'].isin(args.include_mutations)].copy()
    if args.exp_ids:
...
def main() -> None:
    ...
    print("Mutations included:", sorted(block_table["Mutation"].dropna().unique().tolist()))
    print("Sites included:", sorted(block_table["Image_Metadata_Site"].dropna().astype(int).unique().tolist()))
    print("Number of blocks:", len(block_table))
